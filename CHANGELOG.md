...
### 新增：运行日志自动落盘
- `superposition_gui.py`：RunPanel 初始化时打开 `data/log/gui_run.log`，
  所有写入 GUI 运行日志的文本会立即带时间戳追加到该文件。
- 关闭窗口时自动 flush/close。

### 修正：历史记录采样率按证据标记，无证据写 N/A
- 从本会话 wire 日志提取到真实 scope 运行日志：
  - `20260917_234449_c788de`、`20260917_235553_02dc1b` 确实为 **150 MSa/s**
  - `20260918_011148_951dac` 按用户记忆记为 **750 MSa/s**
- 其余无日志、无记忆的历史记录，`awg_sample_rate_ms / bandwidth_mhz / data_rate_mbps` 统一写 `0`，
  GUI 表格显示 **N/A**，避免误传错误值。
- `record.py` 文本摘要中对 0/None 显示 `N/A`。
